...
let Player: React.FC<MeshProps> = forwardRef<Mesh, MeshProps>((props, ref) => {
  ...
  useEffect(() => {
    if (rbRef.current) {
      ...
      // rbRef.current.addForce(new Vector3(0, -1, 0), true);
      // rbRef.current.addForceAtPoint(new Vector3(0, -1, 0), originalDir, true);
    }
  ...
  useFrame(({ clock }) => {
    ...
    if (get().forward) {
      ...
      rbRef.current.setTranslation(characterPosition, true);
    }
    if (get().back) {
      // Create a forward direction vector (e.g., along the negative Z-axis)
